...
def upgrade() -> None:
    ...
    op.create_index('idx_property_amenities_gin', 'properties', ['amenities'], postgresql_using='gin')
    op.create_index('idx_property_features_gin', 'properties', ['features'], postgresql_using='gin')
    op.create_index('idx_vehicle_features_gin', 'vehicles', ['features'], postgresql_using='gin')
    op.create_index('idx_boat_features_gin', 'boats', ['features'], postgresql_using='gin')
    op.create_index('idx_transport_features_gin', 'transportation', ['features'], postgresql_using='gin')
    op.create_index('idx_tour_included_gin', 'tours', ['included'], postgresql_using='gin')
    
    # CHECK Constraints
    op.create_check_constraint('chk_property_rating', 'properties', sa.text('rating >= 0 AND rating <= 5'))
    op.create_check_constraint('chk_property_min_guests', 'properties', sa.text('min_guests > 0'))
    op.create_check_constraint('chk_property_capacity', 'properties', sa.text('max_guests >= min_guests'))
    op.create_check_constraint('chk_review_rating_range', 'reviews', sa.text('rating >= 1 AND rating <= 5'))
    op.create_check_constraint('chk_booking_guests', 'bookings', sa.text('guests > 0'))
    op.create_check_constraint('chk_booking_amount_positive', 'bookings', sa.text('total_amount >= 0'))
    ...
    op.create_index('idx_booking_availability', 'bookings', 
...
def downgrade() -> None:
    ...
    for table, column in tables:
        op.alter_column(table, column, type_=postgresql.JSON(), postgresql_using=f'{column}::json')
