...
def _safe_create_index(
    name: str, table: str, columns, *, postgresql_using: str | None = None
) -> None:
...
def upgrade() -> None:
    ...
    if not _table_exists("room_availability"):
        op.create_table(
            ...
            sa.Column(
                "close_to_arrival", sa.Boolean(), server_default=sa.text("false")
            ),
            sa.Column(
                "close_to_departure", sa.Boolean(), server_default=sa.text("false")
            ),
    ...
    _safe_create_index(
        "idx_room_availability_date_range",
        "room_availability",
        ["room_id", "date", "is_available"],
    )
    _safe_create_index(
        "idx_room_availability_status", "room_availability", ["is_available", "date"]
    )
    _safe_create_index(
        "idx_room_availability_date_lookup",
        "room_availability",
        ["date", "is_available", "room_id"],
    )
    ...
    # ------------------------------------------------------------------
    _safe_create_check(
        "chk_property_rating", "properties", "rating >= 0 AND rating <= 5"
    )
    _safe_create_check("chk_property_min_guests", "properties", "min_guests > 0")
    _safe_create_check(
        "chk_property_capacity", "properties", "max_guests >= min_guests"
    )
    _safe_create_check("chk_property_base_price", "properties", "base_price >= 0")
    _safe_create_check(
        "chk_property_latitude",
        "properties",
        "latitude IS NULL OR (latitude >= -90 AND latitude <= 90)",
    )
    _safe_create_check(
        "chk_property_longitude",
        "properties",
    ...
    _safe_create_check(
        "chk_review_rating_range", "reviews", "rating >= 1 AND rating <= 5"
    )
...
def downgrade() -> None:
    ...
    for name in [
        "chk_tour_rating",
        "chk_tour_price_positive",
        "chk_tour_duration_positive",
        "chk_tour_min_age",
        "chk_tour_group_size",
        "chk_property_rating",
        "chk_property_min_guests",
        "chk_property_capacity",
        "chk_property_base_price",
        "chk_property_latitude",
        "chk_property_longitude",
    ]:
    ...
    for name in [
        ...
        "idx_boat_features_gin",
        "idx_vehicle_features_gin",
        "idx_property_features_gin",
