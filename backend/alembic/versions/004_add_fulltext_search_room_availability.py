...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "notes", sa.Float(), nullable=True
        ),  # Note: Should be Text in actual model
    ...
    op.execute(
        "CREATE INDEX idx_property_fts ON properties USING gin (to_tsvector('spanish', COALESCE(name, '') || ' ' || COALESCE(short_description, '') || ' ' || COALESCE(description, '') || ' ' || COALESCE(city, '') || ' ' || COALESCE(region, '')))"
    )

    # Tour full-text search (Spanish)
    op.execute(
        "CREATE INDEX idx_tour_fts ON tours USING gin (to_tsvector('spanish', COALESCE(name, '') || ' ' || COALESCE(description, '') || ' ' || COALESCE(location, '')))"
    )

    # Blog full-text search (Spanish)
    op.execute(
        "CREATE INDEX idx_blog_fts ON blog_posts USING gin (to_tsvector('spanish', COALESCE(title, '') || ' ' || COALESCE(content, '') || ' ' || COALESCE(excerpt, '')))"
    )
