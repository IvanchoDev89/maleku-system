...
async def get_destinations(
    db: AsyncSession = Depends(get_db), region: str = None, featured: bool = None
):
...
async def get_destination(
    destination_id: uuid.UUID, db: AsyncSession = Depends(get_db)
):
    ...
    await cache.set(
        ...
        tags=["destinations", f"destination:{destination_id}"],
    )
...
async def create_destination(
    ...
    destination = Destination(slug=slug, **destination_data_filtered)
    db.add(destination)
    await db.flush()
    await db.commit()
