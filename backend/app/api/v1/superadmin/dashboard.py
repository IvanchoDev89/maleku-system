...
from app.models import (
    User, UserRole, Vendor, VendorStatus, Booking, BookingStatus,
    Property, Tour, Review, BlogPost, BlogPostStatus, Destination,
...
async def get_dashboard_stats(
    ...
    newsletter_total_result = await db.execute(
        select(func.count(NewsletterSubscriber.id)).where(NewsletterSubscriber.is_active)
    )
...
async def get_newsletter_stats(
    ...
    confirmed_result = await db.execute(
        select(func.count(NewsletterSubscriber.id)).where(NewsletterSubscriber.is_confirmed)
    )
    confirmed = confirmed_result.scalar() or 0
    
    # New today
    new_today_result = await db.execute(
        select(func.count(NewsletterSubscriber.id)).where(NewsletterSubscriber.created_at >= today_start)
    )
    new_today = new_today_result.scalar() or 0
    
    # New this week
    new_week_result = await db.execute(
        select(func.count(NewsletterSubscriber.id)).where(NewsletterSubscriber.created_at >= week_start)
    )
    new_week = new_week_result.scalar() or 0
    
    # New this month
    new_month_result = await db.execute(
        select(func.count(NewsletterSubscriber.id)).where(NewsletterSubscriber.created_at >= month_start)
    )
    new_month = new_month_result.scalar() or 0
    
    # By source
    sources_result = await db.execute(
        select(NewsletterSubscriber.source, func.count(NewsletterSubscriber.id))
