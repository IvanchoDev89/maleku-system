    ...
    async def handle_exception(self, request: Request, exc: Exception):
        ...
        if isinstance(exc, (SQLAlchemyError, IntegrityError)):
            ...
            return JSONResponse(
                ...
                content=_error_content(exc, "A database error occurred. Please try again later.")
            )

        # Validation errors — preserve detail
        if hasattr(exc, 'status_code') and exc.status_code == 422:
            detail = getattr(exc, 'detail', "Invalid input data. Please check your request.")
            return JSONResponse(
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=_error_content(exc, "An unexpected error occurred. Our team has been notified.")
        )
