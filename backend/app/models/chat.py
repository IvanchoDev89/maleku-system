...
from sqlalchemy import Column, String, Boolean, DateTime, ForeignKey, Text, JSON, Enum, Index
...
class Conversation(Base):
    ...
    participant_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=True)
    participant_vendor_id = Column(UUID(as_uuid=True), ForeignKey("vendors.id", ondelete="CASCADE"), nullable=True)
    
    service_type = Column(Enum(ChatServiceType), default=ChatServiceType.GENERAL, nullable=False)
    ...
    # Relationships
    participant_user = relationship("User", foreign_keys=[participant_user_id], back_populates="conversations")
    participant_vendor = relationship("Vendor", foreign_keys=[participant_vendor_id], back_populates="conversations")
