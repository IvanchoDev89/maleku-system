...
class AuditLog(Base):
    ...
    )  # user, vendor, property, booking, etc.
    ...
    __table_args__ = (
        ...
        Index(
            "ix_audit_logs_action_entity_created", "action", "entity_type", "created_at"
        ),
    )
...
class SecurityLog(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "ix_security_logs_failed_logins", "action", "created_at"
        ),  # For detecting brute force
...
class RolePermission(Base):
    ...
    role = Column(
        String(50), nullable=False, index=True
    )  # super_admin, admin, agent, etc.

    # Module/Resource
    module = Column(
        String(50), nullable=False, index=True
    )  # users, vendors, bookings, etc.
    ...
    conditions = Column(
        JSONB, nullable=True
    )  # Conditional permissions, e.g., {"own_data_only": True}
...
class PointOfSale(Base):
    ...
    )  # Hashed API key for POS authentication
