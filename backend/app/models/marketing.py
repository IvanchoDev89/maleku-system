...
from sqlalchemy import Column, String, Text, DateTime, ForeignKey, Integer, Float, Boolean, JSON, Enum, Index
...
class EmailCampaign(Base):
    ...
    # Campaign metadata
    campaign_type = Column(Enum(CampaignType), default=CampaignType.NEWSLETTER, nullable=False)
    ...
    # Targeting
    recipient_type = Column(Enum(RecipientType), default=RecipientType.ALL_USERS, nullable=False)
    vendor_id = Column(UUID(as_uuid=True), ForeignKey("vendors.id"), nullable=True)
    recipient_list = Column(JSONB, nullable=True)  # For INDIVIDUAL type: [user_id, user_id, ...]
    segment_criteria = Column(JSONB, nullable=True)  # For SEGMENT type: {"min_bookings": 5, "location": "..."} 
    ...
    from_email = Column(String(255), nullable=False, default="noreply@costaricatravel.dev")
...
class EmailTemplate(Base):
    ...
    # Template type
    template_type = Column(Enum(CampaignType), default=CampaignType.NEWSLETTER, nullable=False)
...
class EmailLog(Base):
    ...
    # Email details
    subject = Column(String(255), nullable=False)
    message_id = Column(String(255), nullable=True)  # External message ID from BillionMail
    
    # Status tracking
    status = Column(String(50), default="queued", nullable=False)  # queued, sent, delivered, opened, clicked, bounced, failed
...
class MarketingAutomation(Base):
    ...
    trigger_type = Column(String(50), nullable=False)  # user_signup, booking_created, booking_completed, abandoned_cart, etc.
    trigger_criteria = Column(JSONB, nullable=True)  # Additional filter conditions
    
    # Automation steps (sequence of emails)
    steps = Column(JSONB, nullable=False)  # [{"delay_hours": 0, "template_id": "...", "subject": "..."}, ...]
    
    # Status
    is_active = Column(Boolean, default=True)
    
    # Targeting
    vendor_id = Column(UUID(as_uuid=True), ForeignKey("vendors.id"), nullable=True)  # null = global automation
...
class InboxMessage(Base):
    ...
    vendor_id = Column(UUID(as_uuid=True), ForeignKey("vendors.id"), nullable=True)  # null = message to platform admin
    ...
    # Message type
    message_type = Column(String(50), default="inquiry", nullable=False)  # inquiry, support, booking_question, review_reply, etc.
    ...
    # Status
    is_from_customer = Column(Boolean, default=True)  # True = customer sent, False = vendor/admin replied
    ...
    # Relationships
    customer = relationship("User", foreign_keys=[customer_id], back_populates="inbox_messages")
    vendor = relationship("Vendor", foreign_keys=[vendor_id])
...
class EmailPreference(Base):
    ...
    categories = Column(JSONB, default=dict)  # { "deals": true, "new_properties": false, ...}
    
    # Frequency
    email_frequency = Column(String(20), default="daily", nullable=False)  # immediate, daily, weekly, never
