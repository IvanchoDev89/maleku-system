...
class Property(Base):
    ...
    property_type = Column(Enum(PropertyType), default=PropertyType.HOTEL, nullable=False)
    ...
    rooms = relationship("Room", back_populates="property", cascade="all, delete-orphan", lazy="selectin")
    ...
    __table_args__ = (
        ...
        sa.CheckConstraint('latitude IS NULL OR (latitude >= -90 AND latitude <= 90)', name='chk_property_latitude'),
        sa.CheckConstraint('longitude IS NULL OR (longitude >= -180 AND longitude <= 180)', name='chk_property_longitude'),
    )
...
class Room(Base):
    ...
    availability = relationship("RoomAvailability", back_populates="room", cascade="all, delete-orphan", lazy="dynamic")
