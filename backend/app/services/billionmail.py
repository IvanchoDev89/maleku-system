...
class BillionMailService:
    ...
    async def send_email(
        ...
        try:
            payload = {
                ...
                "tracking": {
                    "opens": tracking_enabled,
                    "clicks": tracking_enabled
                }
            }
    ...
    async def create_mailbox(self, email: str, password: str, name: str) -> Dict[str, Any]:
        """
...
class MarketingService:
    ...
    async def create_campaign(
        ...
        from app.models.marketing import EmailCampaign, CampaignStatus, CampaignType, RecipientType
        
        campaign = EmailCampaign(
            name=name,
            subject=subject,
            campaign_type=getattr(CampaignType, campaign_type.upper(), CampaignType.NEWSLETTER),
            status=CampaignStatus.DRAFT,
            html_content=html_content,
            text_content=self.mail_service._strip_html(html_content),
            recipient_type=getattr(RecipientType, recipient_type.upper(), RecipientType.ALL_USERS),
    ...
    async def send_campaign(self, campaign_id: str) -> Dict[str, Any]:
        ...
        if campaign.status != CampaignStatus.DRAFT:
            return {"success": False, "error": f"Campaign is {campaign.status}, not draft"}
        
        # Get recipients based on type
        recipients = await self._get_recipients(campaign.recipient_type, campaign.vendor_id)
    ...
    async def get_campaign_analytics(self, campaign_id: str) -> Dict[str, Any]:
        ...
        stats_result = await self.db.execute(
            select(
                ...
                func.sum(case((EmailLog.status == "clicked", 1), else_=0)).label("clicked"),
